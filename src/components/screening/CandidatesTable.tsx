        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm table-fixed">
            <colgroup>
              ...
              {/* Voice screen — matches the Score column's width so the
                  category columns after it keep their alignment. */}
            ...
            <tbody className="divide-y divide-[#E8E5DF]">
              {loading ? (
                ...
                <tr>
                  <td colSpan={3 + (compact ? 0 : categories.length + 4) + (selectable ? 1 : 0)} className="px-5 py-10 text-center text-sm text-[#737373]">
 ...
 *
 * The score alone would be misleading: a call still ringing, one that never
...
function VoiceCell({ candidate }: { candidate: RankedCandidate }) {
  ...
  return (
    <div className="flex flex-col items-center gap-0.5" title={rec ? `Recommendation: ${rec}` : undefined}>
      ...
      {rec && (
        <span className="text-[10px] capitalize leading-none text-[#737373]">{rec}</span>
      )}
    </div>
  );
}
